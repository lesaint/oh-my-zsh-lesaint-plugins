...
function _custom-mvn()
{
  ...
  # if parameter string contains "-n" we disable multithreaded maven
  if [ "$*" != "$MVN_PARAMS" ]; then
        local MULTITHREAD=""
  fi
...
alias mvn="_custom-mvn"

# aliases
alias mci='mvn clean install -DskipTests'
alias mci-t='mvn clean install'
alias mci-n='mvn -n clean install -DskipTests'

alias mtr='mvn tomcat7:run'
alias mctr='mvn clean tomcat7:run'
alias mtr-d='export MAVEN_OPTS=-agentlib:jdwp=transport=dt_socket,address=8083,server=y,suspend=n; mvn tomcat7:run'
alias mtr-d='export MAVEN_OPTS=-agentlib:jdwp=transport=dt_socket,address=8083,server=y,suspend=n; mvn clean tomcat7:run'
